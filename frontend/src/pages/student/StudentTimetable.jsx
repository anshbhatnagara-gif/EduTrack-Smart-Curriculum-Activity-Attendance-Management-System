...
const StudentTimetable = () => {
  ...
  if (loading) {
    return (
      <div className="space-y-6">
        <PageHeader title="Class Timetable" description="Weekly lecture schedule, room assignments, and subject timings" />
  ...
  const currentSlots = timetable[activeTab] || [];

  return (
    <div className="space-y-6">
      <PageHeader
        title="Class Timetable"
        description="Weekly scheduled lectures, designated room numbers, and subject teachers for your enrolled class section"
      />
        ...
        {currentSlots.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {currentSlots.map((slot) => (
              <div key={slot.id} className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm space-y-3 relative overflow-hidden">
                ...
                <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-600">
                  <div className="flex items-center space-x-1">
                    ...
                    <span>{slot.teacher_name || 'Staff'}</span>
                  </div>
                  <div className="flex items-center space-x-1">
          ...
          <div className="bg-white p-12 rounded-2xl border border-slate-200 text-center space-y-2">
            ...
            <p className="text-xs text-slate-400">Enjoy your free time or check other weekday schedules.</p>
          </div>
        )}
      </div>
    </div>
